...
> Last updated: 2026-08-26 (Session 0/1 — design-level only, no code exists yet; amended Session 5 — migrator-credential placement; amended Session 7 — confirm-payment tenant-context fix and an erasure/dispute-evidence carve-out; amended Session 16 — confirmed by execution that CSRF protection covers the public booking endpoints too, not just authenticated ones; amended Session 17 — a fail-closed owner/staff re-authentication bug found and fixed, D-0043; amended Session 37 — rate limiting built for the public booking endpoint and both login endpoints (D-0063), closing a real, previously-open DoS/brute-force/abuse gap this file had never had a mitigating control for; `manage_booking` token's permanent-validity residual risk (D-0052) closed with a real, bounded expiry (D-0064))

## Amendment (Session 37, 2026-09-19) — rate limiting closes a real, previously uncontrolled abuse surface (D-0063); the `manage_booking` token's indefinite-validity residual risk closed (D-0064)

**Before this session, nothing in this codebase limited the rate of any request** — this file never had a "brute force / credential stuffing" or "resource exhaustion via unauthenticated request flooding" control for either the login endpoints or the public booking-creation endpoint, an omission distinct from (and more basic than) any of this file's existing controls (tenant isolation, CSRF, signed tokens). The public booking endpoint is a real money-adjacent surface — an unthrottled caller could trigger unlimited real Stripe PaymentIntent creations per tenant and exhaust real appointment slots via the hold-window mechanism (FR-03) with no payment ever completed — and neither login endpoint had any brute-force protection at all. Closed via Laravel's own cache-backed named `RateLimiter`, keyed per-credential-and-IP (login) and per-IP-and-tenant (booking); see D-0063 for the full mechanism and threshold reasoning, not duplicated here. This is an additive control — it changes nothing about tenant isolation, CSRF, or the signed-token mechanism this file already documents; a request that would have been allowed before (and was within the rate limit) behaves identically.

**The `manage_booking` token's permanent, no-expiry validity — a residual risk D-0052 (Session 21) named explicitly rather than silently — is now closed.** D-0064 anchors the token's expiry to the appointment's own end time (plus a grace period) rather than issuance time, so a leaked link (forwarded email, shared-device browser history) is no longer a live credential forever — only for the lifetime of the booking it was issued for, plus a bounded grace window. Already-issued tokens (minted before this change, carrying `expires_at: null`) are unaffected and keep their pre-existing exposure — `SignedTenantToken`'s stateless design (D-0021, unchanged) makes retroactively bounding them impossible without a new revocation mechanism this decision deliberately did not build (see D-0064's own "must not be silently reversed" note). This is a forward-looking mitigation, not a retroactive one — worth stating plainly here since this file's job is to say precisely what does and doesn't stop a given risk.
