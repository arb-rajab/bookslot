...
## Amendment (Session 36, 2026-09-18) — owner-admin UI triggers built for all five backend-only endpoints (D-0062)

**Task:** close the gap Session 35's own explicit note (above) queued — real, wired owner-admin UI for refund (D-0056), balance charge (D-0057, plain button per D-0061), Stripe Connect onboarding (D-0058), customer erasure/export (D-0059), and re-invite (D-0060). Not stubs: loading states, per-endpoint error handling, and a confirm step before refund/balance-charge/erasure specifically.

**Placement:** refund and balance-charge landed as two new sections on the existing owner appointment-detail page (`owner/appointments/[id].vue`), directly under "Payments" — exactly where that page's own pre-existing copy already said a refund button would eventually go. Erasure/export/re-invite landed on a new page, `owner/customers/[id].vue`, reached via a new "View customer profile →" link from the appointment-detail page and from the customer-name cell on the appointments list — there's no owner-facing customer-list endpoint in this codebase, so a specific customer is only ever reached from an appointment. `GET /owner/customers/{id}/export` (D-0059) doubles as this page's own data source, so no new read endpoint was needed. Connect onboarding landed at `owner/settings/stripe/index.vue` — the exact route `services.stripe.connect_onboarding_redirect_url` has been configured to point at since D-0058 (Session 28), plus a new "Stripe payouts" nav link. Full reasoning, including the confirm-step design and the balance-charge two-`200`-outcomes-vs-`502` distinction, is in D-0062 (`09-decision-log.md`) — not duplicated here.

**One additive backend field:** `Owner\AppointmentController::present()` gained `customer_id` (shared by `index()`/`show()`) purely so the frontend could build the customer-profile link without a new endpoint. No existing test uses `assertExactJson`, so this is safe by construction; two new assertions were added to prove it's actually populated, not just non-breaking. No other backend logic touched.

**A real bug found by real-browser verification, not by the component tests written first:** the refund section originally hid its own success message the instant the post-refund page reload landed, because the whole section (message included) was gated on there still being a refundable deposit — which goes false the moment the refund actually succeeds. The four new Vitest tests didn't catch this (their mock server always echoed the same static payment status regardless of the refund call); it was found by driving the real page against the real Laravel backend in real Chromium, per this session's own required "test in a browser" step. Fixed, and the regression test strengthened to make its own mock reflect the post-refund state so this class of bug is caught by the fast suite next time too. Full details in D-0062.

**Verification:** `./vendor/bin/pest --exclude-group=queue-broker` 205/205, `--group=queue-broker` 3/3, `./vendor/bin/pint --test` clean, `npx vitest run` 35/35 (16 new), `npx vue-tsc --noEmit` clean, `npx playwright test` (existing suite, unchanged) 10/10. All five new actions additionally driven once, end-to-end, in a real Chromium browser against a freshly reseeded dev database (where the refund bug above was found) — not committed as a new Playwright spec, since this session's own task brief scoped testing to component-level (Vitest) coverage specifically; a future session adding real E2E coverage for these five flows remains a reasonable, not-yet-taken next step.

**Environment:** container was genuinely fresh again (no `.env`, no Postgres roles/databases, no RabbitMQ installed) — every step this file already documents applied unchanged. `composer install --prefer-source` hit the documented `phpstan/phpstan`/`larastan` dist-only-zipball gap; `larastan/larastan` temporarily removed, reverted via `git checkout --` before committing, never committed — `phpstan`/`larastan` not run locally, same as every prior session; real CI installs and runs it normally. `composer audit` reported no vulnerability advisories.

**Docs touched this session:** `09-decision-log.md` (D-0062, header line), `01-scope-and-non-goals.md` (the four "no frontend/Playwright coverage" call-outs for refund/re-invite/balance-charge/Connect updated to point at the now-built UI), `12-session-handoff.md` (this amendment). `05-api-contracts.md` was checked and needed no edit — none of the five endpoints' own contract sections carried "backend-only" framing to update; that framing lived only in `01-scope-and-non-goals.md` and the D-0056/57/58 decision-log entries themselves (left as historical record, not rewritten — this amendment and D-0062 are the forward pointer).

**Remaining backlog:** unchanged from Session 35's list, minus "no owner-admin/settings UI yet for refund, balance-charge, Connect onboarding, erasure/export, or re-invite" (now built, D-0062) — R-01 no real pilot; `bookslot-mobile` cross-repo field-shape check; PR #9 typescript 7.0.2 still parked pending a compatible `vue-tsc` release; no Connect account-replacement flow after a deauthorized account (D-0058); post-appointment rebooking prompt (zero code, unchanged, distinct from re-invite per D-0023); the future auto-charge execution mechanism itself (D-0061 — deferred pending real product/pilot justification); no dedicated Playwright E2E coverage yet for this session's five new UI flows (real-browser-verified once, ad hoc, this session — not committed as a permanent spec, see D-0062).

## Amendment (Session 37, 2026-09-19) — rate limiting built for login and public booking creation (D-0063); `manage_booking` token expiry designed and built (D-0064)

**Task, as given:** close two real, long-named-but-unaddressed gaps — no rate limiting anywhere in the app (login endpoints, and the public booking-creation endpoint that triggers a real Stripe PaymentIntent per call), and `manage_booking` tokens never expiring (D-0052's own named residual risk). Priority order: booking rate limiting first (highest severity — real money-adjacent abuse vector), login rate limiting second, token expiry third (design first, build only if safe and non-breaking).

**All three delivered, not partially.** Rate limiting: Laravel's own named `RateLimiter`/`throttle:` middleware, two limiters (`login`, `booking`) registered in `AppServiceProvider::boot()`, backed by whatever cache store the environment already configures — no new infrastructure. `config/rate_limiting.php` carries the full threshold reasoning. A new `429 { "error": "TOO_MANY_REQUESTS" }` global rendering rule (`bootstrap/app.php`) matches this codebase's existing error-code convention rather than Laravel's default shape. Token expiry: `manage_booking` tokens now expire `booking.manage_booking_token_expiry_grace_days` (3, default) after the appointment's own `ends_at`, computed once at issuance in `BookingController::store()` — anchored to the appointment's own timeline rather than booking-creation time, since a booking can be made up to 60 days ahead and the customer's real need to manage it persists until the appointment itself happens. Full reasoning for both: D-0063/D-0064 (`09-decision-log.md`) — not repeated here.

**The one real risk found and resolved before finalizing thresholds, not assumed away:** a first-pass, textbook 5/minute threshold for both limiters would have broken this repo's own real, passing Playwright E2E suite (`frontend/tests/e2e/`) — it re-authenticates one seeded owner credential ~9 times and creates ~6-7 real bookings against one seeded tenant, all from the same loopback IP, in a single sequential (`workers: 1`) run against one real persistent cache. Found by actually running the full suite against the new middleware (not assumed safe from reading the spec files) — the first run failed all 10 tests. Root-caused two layers deep: first suspected the rate limiter itself, but a `git stash` control run against the unmodified branch reproduced a *different* failure (`MissingAppKeyException` — this container's `.env` had never had `php artisan key:generate` actually complete against a fully-installed `vendor/`, a pre-existing environment-setup gap unrelated to this session's own code, confirmed by reproducing it on stock `main`). Fixed the real `APP_KEY` issue, re-ran clean, and *then* found the genuine rate-limiting/E2E-suite conflict once the environment itself was actually healthy. Raised both thresholds (10/min login-per-credential, 30/min login-per-IP, 10/min booking, 50/hour booking) — verified by re-running the full Playwright suite clean (10/10, ~28s) against the new middleware, not just inferred from arithmetic.

**Backward compatibility for token expiry, by construction, not by added migration code:** `SignedTenantToken` (D-0021) is stateless — a token's `expires_at` claim is baked into its own ciphertext at issuance and can never be changed retroactively. Every `manage_booking` token issued before this change carries `expires_at: null` and keeps resolving forever, completely unchanged — `SignedTenantToken::verify()`'s expiry check was already a no-op for `null` before this session touched anything. No migration, no backfill, no forced-cutover step was written or needed; a dedicated regression test (`ManageBookingControllerTest.php`) travels 5 years into the future to prove this explicitly, guarding against a future session accidentally tightening the verify() logic in a way that would break every already-issued link.

**Tests:** `tests/Feature/Api/RateLimitingTest.php` (new, 6 tests) — per-credential login lockout even with correct credentials on the final attempt, admin login sharing the same limiter, a different credential from the same IP unaffected by another's exhaustion, per-IP spraying-many-emails eventually 429ing, the booking limit tripping (counting malformed requests too, since `throttle:booking` runs before validation), and the booking limit being scoped per-tenant not globally-per-IP. `BookingControllerTest.php` (+1) — the real issued `manage_token` resolves the instant before its computed `ends_at + grace` boundary and is rejected the instant after. `ManageBookingControllerTest.php` (+1) — the legacy null-expiry backward-compatibility guarantee. Backend fast gate: `./vendor/bin/pest --exclude-group=queue-broker` 213/213 (205 baseline + 8 new), `--group=queue-broker` 3/3 unchanged, `./vendor/bin/pint --test` clean. Frontend: `npx vitest run` 35/35 unchanged, `npx vue-tsc --noEmit` clean, full `npx playwright test` 10/10 (the real, load-bearing check for the threshold-calibration finding above).

**Environment:** container was genuinely fresh again (no `.env`, no Postgres roles/databases, no RabbitMQ installed, no `frontend/node_modules`) — every step `CLAUDE.md` documents applied unchanged, plus the `APP_KEY` finding above (worth its own bullet in `CLAUDE.md` if a future session hits the same "migrate:fresh/Pest work fine, but a real `php artisan serve` request 500s with `MissingAppKeyException`" symptom after a `composer install` that failed partway through and was retried). `composer install --prefer-source` hit the documented `phpstan/phpstan`/`larastan` dist-only-zipball gap; `larastan/larastan` temporarily removed from `composer.json`/`composer.lock` (`composer update --prefer-source`), both reverted via `git checkout --` before committing, never committed — `phpstan`/`larastan` not run locally, same as every prior session; real CI installs and runs it normally. `composer audit` reported no vulnerability advisories.

**Docs touched this session:** `09-decision-log.md` (D-0063, D-0064, header line), `05-api-contracts.md` (Session 37 amendment, Deferred section, endpoint 2/3b sections), `06-security-threat-model.md` (Session 37 amendment, header line), `12-session-handoff.md` (this amendment). New file: `config/rate_limiting.php`.

**Remaining backlog:** unchanged from Session 36's list, plus: rate limiting was deliberately not extended beyond login + booking creation (availability/mandate/confirm-payment/manage-booking/all owner-staff-admin routes are untouched — a named scope boundary, D-0063, not an oversight) — revisit if a future session has a concrete reason; the `APP_KEY`/fresh-container gotcha above is worth folding into `CLAUDE.md` proper next time someone hits it again; R-01 no real pilot; `bookslot-mobile` cross-repo field-shape check (this session's `manage_token` expiry change is a real, additive field-shape consideration that repo should be made aware of — the token string itself is opaque to any consumer, but a consumer that cached a `manage_token` indefinitely and expected it to always work would now see it expire; not this repo's problem to fix, but worth flagging to whoever owns `bookslot-mobile`); PR #9 typescript 7.0.2 still parked pending a compatible `vue-tsc` release; post-appointment rebooking prompt (zero code, unchanged); the future auto-charge execution mechanism itself (D-0061 — deferred pending real product/pilot justification); no dedicated Playwright E2E coverage yet for Session 36's five UI flows (D-0062, unchanged).

## Amendment (Session 38, 2026-09-19) — Stripe Connect account-replacement after deauthorization built (D-0065), closing the gap D-0058 itself named and deferred

**Task, as given:** D-0058 (Session 28) built Connect Express onboarding and `account.application.deauthorized` webhook handling, but deliberately left "can a deauthorized tenant reconnect a new Stripe account?" undesigned — every session since (36, 37) carried it forward unchanged in its own "Remaining backlog" line. This session's job was to investigate what reconnection actually requires (rather than assume), design it as a new ADR, implement the minimal correct fix, and prove it end to end.

**The investigation found the gap was worse than "undesigned" — it was actively broken.** `onboardingLink()` only creates a new Stripe account when `tenants.stripe_connect_account_id` is null; D-0058 deliberately left that column populated with the old account id after deauthorization ("a true historical record"). Calling the endpoint again therefore skipped account creation and tried to issue a fresh Account Link against a Stripe account the platform's access to had already been revoked — a call that can only fail, surfacing as a `502 PAYMENT_PROVIDER_UNAVAILABLE` indistinguishable from a transient outage, with no code path that ever recovers. The live status-check endpoint would fail identically. So a real deauthorized tenant had no way to ever reconnect through this app — not merely a missing feature, a dead end.

**Fix, deliberately minimal per the task's own framing:** `ProcessStripeWebhookJob::handleAccountDeauthorized()` now clears `stripe_connect_account_id` back to null and marks the tenant a new, distinct `deauthorized` status (a fifth CHECK-constraint value, migration `2026_09_19_000024`) instead of reusing `restricted`. Once the account id is null, `onboardingLink()`'s existing "create an account if missing" branch is *already* the entire reconnection mechanism — no new endpoint, no new controller method, no new frontend route. `deauthorized` needed to be distinct from `restricted` (not reused) because the two mean genuinely different things to an owner reading the settings page: `restricted` is a still-connected, still-fixable account; `deauthorized` means that account is gone and a *different* one is needed — collapsing them would have owners reading "fix this account" guidance for an account that no longer exists. `Owner\StripeConnectController::status()`'s existing null-account-id short-circuit gained one line to report `deauthorized` (not a bare `not_started`) when that's the tenant's last recorded status, preserving that distinction for the frontend.

**A second, independent correctness gap found and closed along the way:** `account.updated` resolves its tenant via the Stripe Account object's own persisted `metadata.tenant_id`, which survives disconnection (unlike `stripe_connect_account_id`, which this session's fix now clears). A late-arriving `account.updated` for the *old*, disconnected account could previously have overwritten a tenant's post-reconnection status with the old account's stale fields — exactly the "state tied to the old account during the gap" question this task asked about. Fixed with a single equality check in `handleAccountUpdated()` comparing the event's own `data.object.id` against the tenant's *current* `stripe_connect_account_id`, skipping (and logging) a mismatch. Checked and found no equivalent gap for already-settled `payments`/`refunds` rows — they store their own Stripe ids at write time and hold no live reference to the tenant's current Connect account, so clearing that column retroactively corrupts nothing there.

**One residual gap found and deliberately left alone, per this task's own "do not touch refund/balance-charge logic" instruction:** a *new* refund/balance-charge attempt made during the gap (deauthorized, not yet reconnected) doesn't error — `StripePaymentIntentGateway::chargeOffSession()`/`create()` already treat a null connected-account id as "omit `transfer_data`/`application_fee_amount` entirely," so the charge silently succeeds against the platform's own Stripe account instead. This is pre-existing, unchanged behavior (identical to what already happens for any tenant that has never completed onboarding at all), not a new regression — flagged in D-0065 for a future session's attention rather than fixed here, since fixing it would mean touching `Owner\AppointmentController::refund()`/`chargeBalance()`, explicitly out of this session's scope.

**Owner-admin UI (Session 36, D-0062):** `frontend/app/pages/owner/settings/stripe/index.vue` gained a `deauthorized` case in its status-label/action-label maps and one new warning paragraph clarifying that reconnecting doesn't affect past bookings/payment history — the underlying button still calls the exact same `POST .../onboarding-link` action D-0058 already built for every non-`not_started` status; only the copy differs. `frontend/app/types/owner.ts`'s `ConnectStatus` union gained the new value. Verified in a real Chromium browser (a throwaway Playwright spec, per this file's own Session 36 convention — written, run, and deleted before committing, not left in the tree): seeded the demo tenant into the exact post-deauthorization state via `php artisan tinker`, confirmed the distinct "Disconnected" badge/message/button label render correctly, then drove a real click through the real onboarding-link endpoint and confirmed via a follow-up `tinker` query that the tenant's status flipped to `pending` with a freshly-provisioned account id — not just that the right label showed up.

**Tests:** `tests/Feature/Api/StripeWebhookControllerTest.php` — the existing D-0058 deauthorization test updated to expect `deauthorized`/cleared-account-id instead of `restricted`; a new test proving a stale `account.updated` for a tenant's previous, no-longer-current account id is skipped without disturbing its current status, alongside the matching-account-id case still applying normally. `tests/Feature/Api/OwnerStripeConnectControllerTest.php` — a new test for `status()`'s `deauthorized`-vs-`not_started` distinction, and a full deauthorization-to-reconnection regression test driving both `onboarding-link` and `status` through the real HTTP routes against a tenant seeded into the post-deauthorization state, proving a brand new account id is provisioned and the tenant reaches `pending` with no manual intervention. Full fast gate (`./vendor/bin/pest --exclude-group=queue-broker`): 216/216 (213 baseline + 3 new). `--group=queue-broker`: 3/3 unchanged. `tests/TenantIsolation`: 20/20 unchanged. `pint --test` clean. Frontend: `npx vue-tsc --noEmit` clean, `npx vitest run` 35/35 unchanged, full `npx playwright test` (existing suite, unchanged) 10/10.

**Environment:** container was genuinely fresh again (no `.env`, no Postgres roles/databases, no RabbitMQ installed, no `frontend/node_modules`) — every step `CLAUDE.md` documents applied unchanged. `composer install --prefer-source` hit the documented `phpstan/phpstan`/`larastan` dist-only-zipball gap; `larastan/larastan` temporarily removed from `composer.json`/`composer.lock` (`composer update --prefer-source`), both reverted via `git checkout --` before committing, never committed — `phpstan`/`larastan` not run locally, same as every prior session; real CI installs and runs it normally. `composer audit` reported no vulnerability advisories.

**Docs touched this session:** `09-decision-log.md` (D-0065, header line), `04-data-model.md` (`tenants` table's two Connect columns), `05-api-contracts.md` (endpoint 10 section, webhook table row), `01-scope-and-non-goals.md` (the Connect item's own "no account-replacement flow" line marked built), `12-session-handoff.md` (this amendment). New file: `database/migrations/2026_09_19_000024_add_deauthorized_to_tenants_stripe_onboarding_status.php`.

**Remaining backlog:** unchanged from Session 37's list, minus "no Connect account-replacement flow after a deauthorized account" (now built, D-0065), plus: the new residual gap named above (a refund/balance-charge attempt during the deauthorized-but-not-yet-reconnected gap silently routes to the platform's own Stripe account rather than erroring or blocking — pre-existing behavior for any never-onboarded tenant too, not a regression, but worth a future session's attention if real money reconciliation ever matters); R-01 no real pilot; `bookslot-mobile` cross-repo field-shape check; PR #9 typescript 7.0.2 still parked; post-appointment rebooking prompt (zero code, unchanged); the future auto-charge execution mechanism itself (D-0061, still deferred); no dedicated Playwright E2E coverage yet for Session 36's five UI flows or this session's reconnect UI (verified once, ad hoc, in a real browser — not committed as a permanent spec, same convention D-0062 already established).

## Amendment (Session 39, 2026-09-19) — refund/balance-charge misrouting during the Connect deauthorization gap fixed (D-0066), closing the exact residual gap Session 38 named and deferred

**Task, as given:** Session 38's own entry (immediately above) named a real gap and deliberately left it unfixed, per that session's own scope instruction: a refund or off-session balance charge attempted against a tenant with `stripe_connect_account_id === null` (never onboarded, or deauthorized per D-0065 and not yet reconnected) doesn't error — it silently succeeds against the **platform's own** Stripe account instead of the tenant's, since `StripePaymentIntentGateway::create()`/`chargeOffSession()` both treat a null connected-account id as "omit `transfer_data`/`application_fee_amount` entirely." This session's job was to fix it for real, per D-0027/D-0056/D-0057's own standing seriousness about anything touching money, with a real regression test proving the gap first.

**Reproduced first, per this task's own instruction, before touching any application code:** wrote six new Feature tests (three per endpoint — deauthorized, never-onboarded, and a `restricted`-tenant control case) and ran the misrouting ones against the unmodified controller: both returned a plain `200` (the refund/charge "succeeded"), confirming the gap exists exactly as Session 38 described it, not just as a theoretical reading of the code. A genuinely useful side-finding from writing these: `ownerAndTenant()`, this test file's own shared fixture helper, created every tenant with `stripe_connect_account_id` null by `Tenant::factory()`'s own default — meaning the *entire pre-existing test suite for both endpoints* had been unknowingly exercising this exact misrouting condition the whole time, never once asserting on it. That's about as direct a confirmation as this gap could get without a real Stripe account.

**Fix:** both `Owner\AppointmentController::refund()` and `::chargeBalance()` now look up the appointment's tenant and reject with `409 STRIPE_ACCOUNT_NOT_CONNECTED` whenever `stripe_connect_account_id` is null — before any Stripe call is attempted, following this controller's own existing pattern of pre-Stripe-call 409 eligibility gates (`DEPOSIT_NOT_CAPTURED`, `BALANCE_ALREADY_SETTLED`, `PAYMENT_METHOD_NOT_AVAILABLE`). Gated on the column directly, not `stripe_onboarding_status === 'deauthorized'` (one of two options the task brief itself floated) — the column is both more precise (a status-string check would miss the equally-real `not_started` case entirely) and more general (it correctly leaves `restricted` tenants alone, since their account id stays non-null and their refunds/charges still correctly route to their own, still-connected account). `ownerAndTenant()` now creates a properly-connected tenant by default (`stripe_connect_account_id: 'acct_fake_connected'`, `stripe_onboarding_status: 'complete'`); the new gap-specific tests construct their own disconnected tenant directly.

**Historical-risk assessment, done rather than skipped (this task's own item 3):** this codebase has never made a real Stripe API call in any session of its history — D-0036 permanently and deliberately descoped ever obtaining real Stripe credentials for this portfolio project, so every environment any session has ever run in has had only a placeholder-shaped `STRIPE_SECRET_KEY`. No evidence exists, and none is expected to exist, of any real money having moved through any Stripe account — misrouted or otherwise — from this project's own development history. This is *not* a claim about any real-world deployment outside what these sessions have visibility into: if `bookslot` (or a fork) has ever run against real Stripe credentials elsewhere, D-0066's full entry names exactly what to check in the real Stripe dashboard (a transfer/application-fee-less charge or refund against a `payments`/`refunds` row recorded while `stripe_connect_account_id` was null). No attempt was made to reverse or correct any historical Stripe transaction, per this task's explicit instruction — this is a risk-surfacing statement, not a remediation.

**Owner-admin UI (Session 36, D-0062):** `frontend/app/pages/owner/appointments/[id].vue`'s shared `describeError()` — already used by both the refund and balance-charge sections — gained a specific `STRIPE_ACCOUNT_NOT_CONNECTED` message ("go to Settings → Stripe to (re)connect before trying this again") instead of falling through to the generic `Something went wrong (${code})` catch-all. No new component or state needed; both actions already funnel every non-2xx response through this one function.

**Tests:** `tests/Feature/Api/OwnerAppointmentControllerTest.php` — six new tests as described above (reproduced-then-fixed for the misrouting cases, plus the `restricted`-tenant control case proving no over-blocking). `frontend/tests/unit/ownerAppointmentRefundForm.test.ts`/`ownerAppointmentBalanceChargeForm.test.ts` — one new test each confirming the specific message renders. Full fast gate (`./vendor/bin/pest --exclude-group=queue-broker`): 222/222 (216 baseline + 6 new). `--group=queue-broker`: 3/3 unchanged. `tests/TenantIsolation`: 20/20 unchanged. `pint --test` clean. Frontend: `npx vue-tsc --noEmit` clean, `npx vitest run` 37/37 (35 baseline + 2 new).

**Environment:** container was genuinely fresh again (no `.env`, no Postgres roles/databases, no RabbitMQ installed, no `frontend/node_modules`) — every step `CLAUDE.md` documents applied unchanged, including the documented `phpstan/phpstan`/`larastan` dist-only-zipball `composer install --prefer-source` workaround (reverted via `git checkout --` before committing, never committed).

**Docs touched this session:** `09-decision-log.md` (D-0066, header line), `05-api-contracts.md` (endpoints 5 and 6's error lists), `12-session-handoff.md` (this amendment). No new migration — this fix is application-layer only, no schema change.

**Deliberately not touched, per this task's own scope:** `BookingController`'s own deposit-creation call to `PaymentIntentGateway::create()` has the structurally identical gap (a tenant with no connected account gets its first deposit routed to the platform account too) — pre-existing, unrelated to the deauthorization gap specifically, and out of this task's stated scope (refund/balance-charge only). The D-0065 reconnection flow itself (`ProcessStripeWebhookJob`, `Owner\StripeConnectController`) was not touched, per this task's explicit instruction.

**Remaining backlog:** unchanged from Session 38's list, minus the refund/balance-charge misrouting gap named there (now closed, D-0066), plus: the newly-named `BookingController`/deposit-creation instance of the identical gap (above); R-01 no real pilot; `bookslot-mobile` cross-repo field-shape check; PR #9 typescript 7.0.2 still parked; post-appointment rebooking prompt (zero code, unchanged); the future auto-charge execution mechanism itself (D-0061, still deferred); no dedicated Playwright E2E coverage yet for Session 36's five UI flows or Session 38's reconnect UI.

## Amendment (Session 40, 2026-09-26) — deposit-creation misrouting fixed (D-0067), closing the exact residual gap Session 39 named and deferred

**Task, as given:** Session 39's own entry (immediately above) named a real gap and deliberately left it unfixed, per that session's own scope instruction: `BookingController::store()` — the public, unauthenticated, customer-facing endpoint that creates a tenant's first deposit PaymentIntent — never checked `stripe_connect_account_id` before calling Stripe, so a tenant with no connected account (never onboarded, or deauthorized per D-0065 and not yet reconnected) would have a real customer's deposit silently routed to the **platform's own** Stripe account instead of failing. This session's job was to fix it for real, with a real regression test proving the gap first, and to design the error response as customer-facing (this endpoint is public, unlike D-0066's owner-admin refund/balance-charge).

**Reproduced first, per this task's own instruction, before touching any application code:** wrote three new Feature tests in `tests/Feature/Api/BookingControllerTest.php` (never-onboarded, deauthorized, and a `restricted`-tenant control case) and ran the two misrouting ones against the unmodified controller — both returned a plain `201` (the booking "succeeded," a deposit PaymentIntent created), confirming the gap exists exactly as Session 39 described it. Same side-finding shape as D-0066 itself found: this file's own `bookingTenant()` helper had been creating every tenant with `stripe_connect_account_id` null by `Tenant::factory()`'s own default — the entire pre-existing booking-creation test suite had been unknowingly exercising this exact misrouting condition on every run, never once asserting on it.

**Fix:** `BookingController::store()` now looks up the tenant and rejects with `409 BOOKING_UNAVAILABLE` whenever `stripe_connect_account_id` is null — placed after the `pending_payment` hold is taken but before the mandate is rendered or Stripe is called, the same pre-Stripe-call gate discipline D-0066 established. The already-claimed hold is released the same way a genuine Stripe failure already releases it a few lines below, with a new, distinct `cancelled_reason: 'stripe_account_not_connected'` (not `payment_provider_error`, since this isn't a transient condition a retry can fix). Gated on the column directly, not the status string, for the identical reasons D-0066 gives.

**Customer-facing error design, per this task's own item 3:** `BOOKING_UNAVAILABLE` is a new, deliberately generic, provider-agnostic code — this session did *not* reuse D-0066's `STRIPE_ACCOUNT_NOT_CONNECTED` or its "go reconnect Stripe in Settings" copy, since the customer booking an appointment has no Settings page and no reason to be told "Stripe"/"Connect" by name. `409`, matching this controller's own standing convention that a pre-Stripe-call eligibility gate is always `409` (reserving `502 PAYMENT_PROVIDER_UNAVAILABLE` for a genuine failed Stripe *call*). `frontend/app/pages/tenants/[slug]/index.vue`'s `describeError()` gained one new entry: "This studio isn't able to accept online bookings right now. Please contact them directly to book."

**A real test-fixture bug found and fixed in the process, not just the controller's own file:** `tests/Support/concurrency/setup.php` (the real, separate-OS-process fixture `BookingConcurrencyTest.php`'s true multi-connection race test uses) also needed a connected tenant, but reusing the same fixed `'acct_fake_connected'` literal `bookingTenant()`/`ownerAndTenant()` use broke the full suite: that fixture's insert is a genuine, permanent commit from a standalone process outside any Pest `RefreshDatabase` transaction, so — unlike every other test's own fixture — it is never rolled back. A full-suite run reproducibly failed 40 unrelated tests with a real `tenants_stripe_connect_account_id_unique` violation the moment this was tried. Fixed with a random-per-run id (`'acct_fake_concurrency_'.Str::random(16)`) instead. Worth flagging for any future session adding a new fixed Stripe-account-id literal anywhere in this codebase: it's safe in an ordinary (rolled-back) test fixture, but never safe in one of this suite's few genuinely-committing real-subprocess fixtures.

**A second ripple, caught only by CI's own real-browser E2E job, not by anything runnable locally in isolation from GitHub Actions:** `database/seeders/DatabaseSeeder.php`'s `demo-studio` tenant — the one every Playwright E2E spec that books a real appointment books against, per that seeder's own docblock — had `stripe_onboarding_status: 'not_started'` and no connected account since before Stripe Connect (D-0058) existed. Pest never seeds it (self-contained fixtures per test) and Vitest never touches a real backend, so this PR's own first CI run was the first thing to actually exercise it: `booking-flow.spec.ts` failed waiting for "Confirm your deposit" to appear, because the real API now correctly returns `409 BOOKING_UNAVAILABLE` for this tenant. Fixed by seeding `stripe_connect_account_id: 'acct_demo_seed_connected'`, `stripe_onboarding_status: 'complete'` — confirmed locally afterward with a fresh `migrate:fresh --seed` plus `npx playwright test tests/e2e/booking-flow.spec.ts` (2/2) and the full E2E suite (10/10 on one run). A second full-suite run hit one `TOO_MANY_REQUESTS` on a `loginAsOwner()` call in a different spec — traced to D-0063's login rate limiter accumulating across repeated full-suite runs against the same long-lived local dev server, confirmed as a pre-existing, unrelated flake (not tied to this PR's diff) by re-running that spec file alone (4/4 clean) and by two different tests in that file each being the one to trip it across two separate full-suite runs.

**Historical-risk assessment, done rather than skipped (this task's own item 4):** this codebase has never made a real Stripe API call in any session of its history — D-0036 permanently and deliberately descoped ever obtaining real Stripe credentials for this portfolio project. No evidence exists, and none is expected to exist, of any real customer deposit — misrouted or otherwise — having actually moved real money through any Stripe account, platform or tenant's, at any point in this project's own development history. This is *not* a claim about any real-world deployment outside what these sessions have visibility into: if `bookslot` (or a fork) has ever run against real Stripe credentials elsewhere, D-0067's full entry names exactly what to check in the real Stripe dashboard (a deposit charge with no destination transfer, tied to a `payments` row of `type = 'deposit'` recorded while the tenant's `stripe_connect_account_id` was null). No attempt was made to reverse or correct any historical Stripe transaction, per this task's explicit instruction.

**Environment:** container was genuinely fresh again (no `.env`, no vendor/, no Postgres roles/databases, no RabbitMQ installed, no `frontend/node_modules`) — every step `CLAUDE.md` documents applied unchanged, including the documented `phpstan/phpstan`/`larastan` dist-only-zipball `composer install --prefer-source` workaround (reverted via `git checkout --` before committing, never committed).

**Tests:** `tests/Feature/Api/BookingControllerTest.php` — three new tests, `bookingTenant()` now defaults to a connected tenant. `tests/Support/concurrency/setup.php` — random per-run account id, as described above. Full fast gate (`./vendor/bin/pest --exclude-group=queue-broker`): 225/225 (222 baseline + 3 new; includes `BookingConcurrencyTest.php`, confirmed still racing on `SLOT_ALREADY_BOOKED`, not the new gate). `--group=queue-broker`: 3/3 unchanged. `tests/TenantIsolation`: 20/20 unchanged. `pint --test` clean. Frontend: `npx vue-tsc --noEmit` clean, `npx vitest run` 37/37 unchanged (no component-level test file exists for the public booking page at all — flagged, not silently left unstated — its only automated coverage is Playwright's `booking-flow.spec.ts`, which was not extended this session to assert on the new copy specifically).

**Docs touched this session:** `09-decision-log.md` (D-0067, header line), `05-api-contracts.md` (endpoint 2's error list), `12-session-handoff.md` (this amendment). No new migration — application-layer only, no schema change.

**Deliberately not touched, per this task's own scope:** the D-0065 reconnection flow (`ProcessStripeWebhookJob`, `Owner\StripeConnectController`) and D-0066's own refund/balance-charge fixes were not touched.

**Remaining backlog:** unchanged from Session 39's list, minus the deposit-creation misrouting gap named there (now closed, D-0067), plus: no dedicated Vitest component test for the public booking page's new `BOOKING_UNAVAILABLE` copy (Playwright's happy-path-only E2E suite is this page's sole coverage); R-01 no real pilot; `bookslot-mobile` cross-repo field-shape check; PR #9 typescript 7.0.2 still parked; post-appointment rebooking prompt (zero code, unchanged); the future auto-charge execution mechanism itself (D-0061, still deferred); no dedicated Playwright E2E coverage yet for Session 36's five UI flows or Session 38's reconnect UI.
